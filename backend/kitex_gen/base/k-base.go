...
func (p *TrafficEnv) DeepCopy(s interface{}) error {
	src, ok := s.(*TrafficEnv)
	if !ok {
	...
	if src.Env != "" {
		p.Env = kutils.StringDeepCopy(src.Env)
	}
...
func (p *Base) DeepCopy(s interface{}) error {
	src, ok := s.(*Base)
	if !ok {
		return fmt.Errorf("%T's type not matched %T", s, p)
	}
	...
	if src.LogID != "" {
		p.LogID = kutils.StringDeepCopy(src.LogID)
	}

	if src.Caller != "" {
		p.Caller = kutils.StringDeepCopy(src.Caller)
	}

	if src.Addr != "" {
		p.Addr = kutils.StringDeepCopy(src.Addr)
	}

	if src.Client != "" {
		p.Client = kutils.StringDeepCopy(src.Client)
	}

	var _trafficEnv *TrafficEnv
	if src.TrafficEnv != nil {
		_trafficEnv = &TrafficEnv{}
		if err := _trafficEnv.DeepCopy(src.TrafficEnv); err != nil {
			return err
		}
	}
	p.TrafficEnv = _trafficEnv
...
func (p *BaseResp) DeepCopy(s interface{}) error {
	...
	if src.StatusMessage != "" {
		p.StatusMessage = kutils.StringDeepCopy(src.StatusMessage)
	}
